...
from data.old_code.fetch import firefighter_stations_data, fire_incidents_data
...
def create_choropleth_stations() -> None:
    ...
    gdf = gpd.GeoDataFrame(
        data=fire_stations,
        geometry=gpd.points_from_xy(fire_stations.LONGITUDE, fire_stations.LATITUDE),
    )
...
def create_fire_incidents_yearly():
    ...
    grid = create_grid(
        geo_data=lim_admin_mtl, distance=500, units=Unit.METERS, save_as_file=True
    )
    ...
    firestation_grouped = (
        firestation_geo.groupby("CODEMAMH").size().to_frame("TOT_CASERNES")
    )
    firestation_geo = firestation_geo.join(
        firestation_grouped, on="CODEMAMH", how="left"
    )
    ...
    fields = ["Grid Name", "TOT_INC_GRID"] + incident_by_year.columns.values.astype(
        str
    ).tolist()
    aliases = ["Grid Name: ", "Total Fires: "] + incident_by_year.columns.values.astype(
        str
    ).tolist()
    ...
    for idx in range(len(fire_stations)):
        folium.Circle(
            location=(
                firestation_geo.loc[idx, "LATITUDE"],
                firestation_geo.loc[idx, "LONGITUDE"],
            ),
            radius=50,
            color="#3189FF",
            fill=True,
